...
5. **Marathon and Night Owl — retired**, per founder decision. Removed from `ACHIEVEMENT_DEFINITIONS`; their now-dead unlock logic deleted from `achievement.service.ts` rather than left as inert code. No data migration needed — `getAchievementsFn` only maps over the definitions array, so any historically-unlocked rows simply stop being surfaced.
   ...
   **2026-08-01 reversal**: reinstated by founder decision as part of the Journey Mode gamified-map feature, with the original triggers restored unchanged (Marathon: 5+ sessions/day; Night Owl: after 8pm). The wellbeing risk this section raised was not re-argued or mitigated as part of the reversal — it was a deliberate choice to prioritize the map's checkpoint completeness, made with the original finding in view, not overlooked. Revisit the counter-signal option in [12_Gamification_Framework.md](12_Gamification_Framework.md) §4 if the wellbeing concern needs to be addressed without removing the badges again.
