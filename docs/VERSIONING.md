...
| v1.0 (Sprint 5) | 2026-08-01 | **Complete** | Closed Version 1.0's XP-reconciliation item — `users.xp` deleted outright (confirmed zero readers anywhere in the app before removing it, not a formalization exercise); `xp_ledger` is the sole source of truth. No speculative read helper added, since nothing yet displays a total XP figure. A real root cause tracked down for a repeatedly-flaky e2e step (route-warming via unauthenticated `curl` never actually compiled the expensive component code behind a `beforeLoad` redirect) rather than dismissed again as generic flakiness. See [05_Sprint_5_Review.md](sprints/05_Sprint_5_Review.md). |
