...
// reproduces. Idempotent-ish: re-running clears prior pilot-demo rows by
// email prefix first (see CLEAN UP below) rather than duplicating.
...
async function main() {
  console.log('--- Cleaning up any previous pilot-demo run ---')
  ...
  console.log(`Removed ${staleUsers.length} stale demo users.`)
  ...
  console.log('--- Creating activities (quizzes) ---')
  ...
  console.log('--- Generating 14-day activity history per student ---')
  ...
  for (let idx = 0; idx < students.length; idx++) {
    ...
    const isStar = idx < 2 // first two students get a near-continuous streak for a strong demo example
    ...
    await adminDb.transaction(async (tx) => {
      ...
      for (let day = 0; day < PILOT_DAYS; day++) {
        ...
        const progress = day / (PILOT_DAYS - 1) // 0 -> 1 across the pilot
        ...
        if (day % 4 === 2 && Math.random() < 0.6) {
          const moodBase = 2 + Math.round(progress * 2) // trends from ~2 to ~4
  ...
  console.log('--- Done ---')
  console.log(`Institution: ${institution.name} (${institution.campus})`)
  console.log(`Teachers: 2, Students: ${students.length}, Classes: 4, Activities: 3`)
  console.log(`Focus sessions: ${totalSessions}, Tasks: ${totalTasks} (${totalCompletedTasks} completed), Quiz attempts: ${totalAttempts}`)
